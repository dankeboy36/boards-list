...
/**
 * Representation of a detected port with multiple discovered boards on the same
 * port. For example Arduino Nano ESP32 from `esp32:esp32:nano_nora` and
 * `arduino:esp32:nano_nora`. If multiple boards are detected, but the board
 * names are the same, the `board` will be the `first` element of the `boards`
 * array. If multiple boards are detected, but the board names are not
 * identical, the `board` will be missing.
 */
...
/**
 * Base inferred board list item type. The the type of the inferred board can
 * be:
 *
...
/**
 * No boards have been discovered for a detected port, it has been manually
...
/**
 * One or more boards have been discovered for a detected port, but the board
 * has been overridden by a manual action.
 */
...
function getBoardOrInferredBoard(
  ...
  let board: BoardIdentifier | undefined
  board = item.board
...
const inferenceTypeLiterals = [
  /**
   * The user has manually selected the board (FQBN) for the detected port of a
   * 3rd party board (no matching boards were detected by the CLI for the port)
   */
  'manually-selected',
  /**
   * The user has manually edited the detected FQBN of a recognized board from a
   * detected port (there are matching boards for a detected port, but the user
   * decided to use another FQBN)
   */
  'board-overridden',
] as const
 ...
 * Compare precedence:
 *
 * 1. `BoardsListItem#port#protocol`: `'serial'`, `'network'`, then natural compare
 *    of the `protocol` string.
 * 2. `BoardsListItem`s with a `board` comes before items without a `board`.
 * 3. `BoardsListItem#board`:
 *
 *    1. Items with `'arduino'` vendor ID in the `fqbn` come before other vendors.
 *    2. Natural compare of the `name`.
 * 4. If the `BoardsListItem`s do not have a `board` property:
 *
 *    1. Ambiguous boards come before no boards.
 *    2. `BoardsListItem#port#address` natural compare is the fallback.
 */
...
/**
 * What is show in the UI for a particular board with all its refinements,
 * fallbacks, and tooltips.
 */
...
/**
 * A list of boards discovered by the Arduino CLI. With the `board list --watch`
 * gRPC equivalent command, the CLI provides a `1..*` mapping between a port and
 * the matching boards list. This type inverts the mapping and makes a `1..1`
 * association between a board identifier and the port it belongs to.
 */
export interface BoardsList {
  readonly labels: BoardsListLabels
  /**
   * All detected ports with zero to many boards and optional inferred
   * information based on historical selection/usage.
   */
  readonly items: readonly BoardsListItemUI[]
  /**
   * A snapshot of the board and port configuration this board list has been
   * initialized with.
   */
  readonly boardsConfig: Readonly<BoardsConfig>

  /**
   * Index of the board+port item that is currently "selected". A board list
   * item is selected, if matches the board+port combination of `boardsConfig`.
   */
